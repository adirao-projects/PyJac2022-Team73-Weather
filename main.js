...
eel.expose(setCities);
function setCities (city_dat) {
    console.log(city_dat);
    var city_name = city_dat[0];
    var temp = city_dat[1].temp;
    console.log(city_dat[1].temp);
    var cloud = city_dat[2];
    var wind = city_dat[3].speed;

    var html = '';
    //Gets data from python (id and name) and adds buttons to the html DOM. Each buttton's onclick sends the client id to python

    html = '<button onclick="'
    + "cityData("+city_name+");"+'" class="card" >'
    +city_name+"<br>"
...
function searchCity() {
    ...
    eel.get_weather(query)(setCities);
}
...
/*Sents data to python*/
function eelTester() {
    var info = document.getElementById("pyIn").value;
    eel.dummy(info)(eelTesterpy);
}
function eelTester2() {
    eel.dummy(5)(eelTesterpy);
}

/*Gets return from python*/
eel.expose(eelTesterpy);
function eelTesterpy(data, data2) {
    console.log(data);
    console.log(data2);
    document.getElementById("jsOut").innerHTML = data[0];
}

eel.expose(labelSetter);
function labelSetter(data) {
